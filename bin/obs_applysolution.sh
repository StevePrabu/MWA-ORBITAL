...
usage()
{
echo "applysolutions.sh [-o obsnum] [-c calibration sol]
...
## run template script
script="${MYBASE}/queue/applysolution_${obsnum}.sh"
...
sub="sbatch --begin=now+15 --output=${output} --error=${error} ${depend} -J applysolution_${obsnum} -M ${MYCLUSTER} ${script}"
jobid=($(${sub}))
jobid=${jobid[3]}
